# Third-party dependency licences

> Generated by build/check-licenses.ps1 — do not edit by hand.
> Regenerate with:  pwsh build/check-licenses.ps1 -Update
> Every licence below must be in the SPEC §2 allowed list; the build fails otherwise.

| Package | Version | Licence |
|---|---|---|
| Azure.Core | 1.50.0 | MIT |
| Castle.Core | 5.1.1 | Apache-2.0 |
| CommunityToolkit.Mvvm | 8.4.2 | MIT |
| DiffEngine | 11.3.0 | MIT |
| EmptyFiles | 4.4.0 | MIT |
| H.GeneratedIcons.System.Drawing | 2.4.1 | MIT |
| H.NotifyIcon | 2.4.1 | MIT |
| H.NotifyIcon.Wpf | 2.4.1 | MIT |
| Microsoft.ApplicationInsights | 2.23.0 | MIT |
| Microsoft.Bcl.AsyncInterfaces | 8.0.0 | MIT |
| Microsoft.CodeAnalysis.BannedApiAnalyzers | 5.6.0 | MIT |
| Microsoft.CodeCoverage | 18.9.0 | MIT |
| Microsoft.Data.Sqlite | 10.0.11 | MIT |
| Microsoft.Data.Sqlite.Core | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration.Abstractions | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration.Binder | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration.CommandLine | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration.EnvironmentVariables | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration.FileExtensions | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration.Json | 10.0.11 | MIT |
| Microsoft.Extensions.Configuration.UserSecrets | 10.0.11 | MIT |
| Microsoft.Extensions.DependencyInjection | 10.0.11 | MIT |
| Microsoft.Extensions.DependencyInjection.Abstractions | 10.0.11 | MIT |
| Microsoft.Extensions.Diagnostics | 10.0.11 | MIT |
| Microsoft.Extensions.Diagnostics.Abstractions | 10.0.11 | MIT |
| Microsoft.Extensions.FileProviders.Abstractions | 10.0.11 | MIT |
| Microsoft.Extensions.FileProviders.Physical | 10.0.11 | MIT |
| Microsoft.Extensions.FileSystemGlobbing | 10.0.11 | MIT |
| Microsoft.Extensions.Hosting | 10.0.11 | MIT |
| Microsoft.Extensions.Hosting.Abstractions | 10.0.11 | MIT |
| Microsoft.Extensions.Logging | 10.0.11 | MIT |
| Microsoft.Extensions.Logging.Abstractions | 10.0.11 | MIT |
| Microsoft.Extensions.Logging.Configuration | 10.0.11 | MIT |
| Microsoft.Extensions.Logging.Console | 10.0.11 | MIT |
| Microsoft.Extensions.Logging.Debug | 10.0.11 | MIT |
| Microsoft.Extensions.Logging.EventLog | 10.0.11 | MIT |
| Microsoft.Extensions.Logging.EventSource | 10.0.11 | MIT |
| Microsoft.Extensions.Options | 10.0.11 | MIT |
| Microsoft.Extensions.Options.ConfigurationExtensions | 10.0.11 | MIT |
| Microsoft.Extensions.Primitives | 10.0.11 | MIT |
| Microsoft.Graph | 6.5.0 | MIT |
| Microsoft.Graph.Core | 4.0.1 | MIT |
| Microsoft.Identity.Client | 4.87.0 | MIT |
| Microsoft.Identity.Client.Extensions.Msal | 4.87.0 | MIT |
| Microsoft.IdentityModel.Abstractions | 8.18.0 | MIT |
| Microsoft.IdentityModel.JsonWebTokens | 8.18.0 | MIT |
| Microsoft.IdentityModel.Logging | 8.18.0 | MIT |
| Microsoft.IdentityModel.Protocols | 8.18.0 | MIT |
| Microsoft.IdentityModel.Protocols.OpenIdConnect | 8.18.0 | MIT |
| Microsoft.IdentityModel.Tokens | 8.18.0 | MIT |
| Microsoft.IdentityModel.Validators | 8.18.0 | MIT |
| Microsoft.IO.RecyclableMemoryStream | 3.0.1 | MIT |
| Microsoft.Kiota.Abstractions | 2.0.0 | MIT |
| Microsoft.Kiota.Authentication.Azure | 2.0.0 | MIT |
| Microsoft.Kiota.Http.HttpClientLibrary | 2.0.0 | MIT |
| Microsoft.Kiota.Serialization.Form | 2.0.0 | MIT |
| Microsoft.Kiota.Serialization.Json | 2.0.0 | MIT |
| Microsoft.Kiota.Serialization.Multipart | 2.0.0 | MIT |
| Microsoft.Kiota.Serialization.Text | 2.0.0 | MIT |
| Microsoft.NET.Test.Sdk | 18.9.0 | MIT |
| Microsoft.Testing.Extensions.Telemetry | 2.3.3 | MIT |
| Microsoft.Testing.Extensions.TrxReport.Abstractions | 2.3.3 | MIT |
| Microsoft.Testing.Platform | 2.3.3 | MIT |
| Microsoft.Testing.Platform.MSBuild | 2.3.3 | MIT |
| Microsoft.TestPlatform.ObjectModel | 18.9.0 | MIT |
| Microsoft.TestPlatform.TestHost | 18.9.0 | MIT |
| Microsoft.VisualStudio.Threading.Analyzers | 18.7.23 | MIT |
| Microsoft.Win32.Registry | 5.0.0 | MIT |
| Microsoft.Windows.CsWin32 | 0.3.321 | MIT |
| Microsoft.Windows.SDK.Win32Docs | 0.1.42-alpha | build-time only, not redistributed (see exemption note in check-licenses.ps1) |
| Microsoft.Windows.SDK.Win32Metadata | 71.0.14-preview | build-time only, not redistributed (see exemption note in check-licenses.ps1) |
| Microsoft.Windows.WDK.Win32Metadata | 0.13.25-experimental | build-time only, not redistributed (see exemption note in check-licenses.ps1) |
| NSubstitute | 6.2.0 | BSD-3-Clause |
| Serilog | 4.4.0 | Apache-2.0 |
| Serilog.Extensions.Hosting | 10.0.0 | Apache-2.0 |
| Serilog.Extensions.Logging | 10.0.0 | Apache-2.0 |
| Serilog.Sinks.File | 7.0.0 | Apache-2.0 |
| SharpGen.Runtime | 2.4.2-beta | MIT |
| SharpGen.Runtime.COM | 2.4.2-beta | MIT |
| Shouldly | 4.3.0 | BSD-3-Clause |
| SQLitePCLRaw.bundle_e_sqlite3 | 2.1.12 | Apache-2.0 |
| SQLitePCLRaw.core | 2.1.12 | Apache-2.0 |
| SQLitePCLRaw.lib.e_sqlite3 | 2.1.12 | Apache-2.0 |
| SQLitePCLRaw.provider.e_sqlite3 | 2.1.12 | Apache-2.0 |
| Std.UriTemplate | 2.0.8 | Apache-2.0 |
| System.ClientModel | 1.8.0 | MIT |
| System.CodeDom | 6.0.0 | MIT |
| System.CommandLine | 2.0.11 | MIT |
| System.Diagnostics.EventLog | 10.0.11 | MIT |
| System.IdentityModel.Tokens.Jwt | 8.18.0 | MIT |
| System.IO.Hashing | 10.0.11 | MIT |
| System.Management | 6.0.1 | MIT |
| System.Memory.Data | 8.0.1 | MIT |
| System.Security.AccessControl | 6.0.1 | MIT |
| System.Security.Cryptography.ProtectedData | 4.5.0 | MIT |
| Vortice.Direct3D11 | 3.8.3 | MIT |
| Vortice.DirectX | 3.8.3 | MIT |
| Vortice.DXGI | 3.8.3 | MIT |
| Vortice.Mathematics | 2.1.0 | MIT |
| WPF-UI | 4.3.0 | MIT |
| WPF-UI.Abstractions | 4.3.0 | MIT |
| xunit.analyzers | 2.0.0 | Apache-2.0 |
| xunit.runner.visualstudio | 4.0.0 | Apache-2.0 |
| xunit.v3 | 4.0.0 | Apache-2.0 |
| xunit.v3.assert | 4.0.0 | Apache-2.0 |
| xunit.v3.common | 4.0.0 | Apache-2.0 |
| xunit.v3.core.mtp-v2 | 4.0.0 | Apache-2.0 |
| xunit.v3.extensibility.core | 4.0.0 | Apache-2.0 |
| xunit.v3.mtp-v2 | 4.0.0 | Apache-2.0 |
| xunit.v3.runner.common | 4.0.0 | Apache-2.0 |
| xunit.v3.runner.inproc.console | 4.0.0 | Apache-2.0 |

## Bundled binaries (non-NuGet)

| Component | Build | Licence |
|---|---|---|
| FFmpeg (child process, unmodified) | ffmpeg-n8.1.2-44-g7c533d0f86-win64-lgpl | LGPL-2.1-or-later |

FFmpeg licence texts ship alongside the binary; the written source offer is in docs/ffmpeg-source-offer.md.
