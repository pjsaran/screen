...
public sealed class MsalTokenProvider : IAccessTokenProvider
{
    ...
    {
        if (_destination.TenantId is null || _destination.ClientId is null || _destination.CredentialName is null)
        {
            throw new DeliveryException(FailureKind.Permanent,
        ...
        catch (MsalServiceException exception) when (exception.StatusCode is 400 or 401)
        {
            ...
            throw new DeliveryException(FailureKind.AuthExpired, exception.Message);
        }
    }
    ...
    private async Task<IConfidentialClientApplication> BuildAppAsync()
    {
        ...
        IConfidentialClientApplication app = CredentialVault.Use(_destination.CredentialName!, secretBytes =>
