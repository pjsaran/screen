...
10. Much later, `RetentionCleaner` reclaims the disk — but only for sessions that
    are finalised, transferred *and* verified somewhere else, and past the
...
| `Captr.Core/Diagnostics` | The health checks behind the Diagnostics page, and the support bundle (no video, no secrets) |
| `Captr.Core/Cli` | Every command and its documented exit code |
| `Captr.App` | The WPF UI (design system in `Theme/`, five pages, the tray), and the `--host` role switch |
