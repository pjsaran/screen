...
`testing.md` maps every SPEC §14 requirement to the test that covers it,
...
- [x] `build/verify-install.ps1` on the build machine: install → CLI answers →
      real 15 s recording → playable, correctly-named file transferred to a folder
      destination.
...
☐ A Task Scheduler task configured **run only when user is logged on** starts and
  stops a recording (../user-guide/scheduling.md example), and the resulting file is
...
☐ SharePoint: ../user-guide/sharepoint-setup.md against a real tenant — both credential modes;
