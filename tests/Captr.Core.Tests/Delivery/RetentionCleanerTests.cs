...
public class RetentionCleanerTests : IDisposable
{
    ...
    public RetentionCleanerTests()
    {
        ...
        _queue = new DeliveryQueue(Path.Combine(_dir, "delivery.db"));
    ...
    [Fact]
    public void A_delivered_verified_session_past_retention_is_removed()
    {
        string folder = MakeSession("old-delivered", finalizedDaysAgo: 30);
    ...
    {
        string folder = MakeSession("recent-delivered", finalizedDaysAgo: 3);
        long id = _queue.Enqueue(Path.Combine(folder, "output.mkv"), "archive");
        _queue.Complete(id);
    ...
    [Fact]
    public void A_session_that_was_never_delivered_anywhere_is_NEVER_removed()
    {
    ...
    [Fact]
    public void A_session_with_any_incomplete_delivery_is_kept()
    {
        string folder = MakeSession("old-partly-delivered", finalizedDaysAgo: 30);
    ...
    [Fact]
    public void Zero_retention_days_still_requires_a_completed_delivery()
    {
        _settingsStore.Save(_settingsStore.Load() with { RetentionDays = 0 });
        string undelivered = MakeSession("zero-retention-undelivered", finalizedDaysAgo: 1);

        MakeCleaner().Clean(Now).ShouldBeEmpty();

        Directory.Exists(undelivered).ShouldBeTrue();
    }
